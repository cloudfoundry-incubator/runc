...
func TestFactoryLoadContainer(t *testing.T) {
	...
	var (
		...
		expectedHooks = &configs.Hooks{
			Prestart:  []configs.Hook{configs.CommandHook{configs.Command{Path: "prestart-hook"}}},
			Poststart: []configs.Hook{configs.CommandHook{configs.Command{Path: "poststart-hook"}}},
			Poststop:  []configs.Hook{unserializableHook{}, configs.CommandHook{configs.Command{Path: "poststop-hook"}}},
		}
